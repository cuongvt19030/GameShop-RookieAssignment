...
import { FilterGenreOptions } from "../../../Constants/selectOptions";

import { Link } from "react-router-dom";
import GameTable from "./GameTable";

import { GetGameRequest } from "../services/request"
import { 
  ACCSENDING, 
  DECSENDING, 
...
const ListGame = () => {
  ...
  const handleType = (selected) => {
    if (selected.length === 0) {
      ...
      setSelectedType([FilterGenreOptions[0]]);
      return;
    }
  ...
  const fetchDataCallbackAsync = async () =>  {
    let data = await GetGameRequest(query);
  ...
  useEffect(() => {
    async function fetchDataAsync() {
      let result = await GetGameRequest(query);
  ...
  return (
    <>
      <div className="primaryColor text-title intro-x">Game List</div>
